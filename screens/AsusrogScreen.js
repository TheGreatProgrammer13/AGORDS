...
const AsusrogScreen = () => {
  ...
  const [totalPrice, setTotalPrice] = useState(1699.99);
  const { addToCart } = useCart(); // Get the addToCart function from CartContext

  const decreaseQuantity = () => {
    if (quantity > 1) {
      setQuantity(quantity - 1);
      setTotalPrice(totalPrice - 1699.99);
    }
  };
    ...
    >
      <View style={styles.container}>
        <View style={styles.productDetails}>
          {/* Product details */}
          <Text style={styles.productName}>ASUS ROG Strix G16</Text>
          <Image
            source={require('../assets/asus.jpg')}
