...
const ViptechScreen = () => {
  ...
  const [totalPrice, setTotalPrice] = useState(999.99);
  const { addToCart } = useCart(); // Get the addToCart function from CartContext

  const decreaseQuantity = () => {
    if (quantity > 1) {
      setQuantity(quantity - 1);
      setTotalPrice(totalPrice - 999.99);
    }
  };
    ...
    >
      <View style={styles.container}>
        <View style={styles.productDetails}>
          {/* Product details */}
          <Text style={styles.productName}>ViprTech Ghost 3.0</Text>
          <Image
            source={require('../assets/viptech.jpg')}
