...
const MsiScreen = () => {
  ...
  const [totalPrice, setTotalPrice] = useState(1399.99);
  const { addToCart } = useCart(); // Get the addToCart function from CartContext

  const decreaseQuantity = () => {
    if (quantity > 1) {
      setQuantity(quantity - 1);
      setTotalPrice(totalPrice - 1399.99);
    }
  };
    ...
    >
      <View style={styles.container}>
        <View style={styles.productDetails}>
          {/* Product details */}
          <Text style={styles.productName}>MSI Katana A15</Text>
          <Image
            source={require('../assets/msi.jpg')}
