...
const CoolerScreen = () => {
  ...
  const [totalPrice, setTotalPrice] = useState(1129.99);
  const { addToCart } = useCart(); // Get the addToCart function from CartContext

  const decreaseQuantity = () => {
    if (quantity > 1) {
      setQuantity(quantity - 1);
      setTotalPrice(totalPrice - 1129.99);
    }
  };
    ...
    >
      <View style={styles.container}>
        <View style={styles.productDetails}>
          {/* Product details */}
          <Text style={styles.productName}>Cooler Master HAF 5 Pro</Text>
          <Image
            source={require('../assets/cooler.jpg')}
