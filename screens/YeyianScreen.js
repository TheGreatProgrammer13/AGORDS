...
const YeyianScreen = () => {
  ...
  const [totalPrice, setTotalPrice] = useState(1359.99);
  const { addToCart } = useCart(); // Get the addToCart function from CartContext

  const decreaseQuantity = () => {
    if (quantity > 1) {
      setQuantity(quantity - 1);
      setTotalPrice(totalPrice - 1359.99);
    }
  };
    ...
    >
      <View style={styles.container}>
        <View style={styles.productDetails}>
          {/* Product details */}
          <Text style={styles.productName}>Yeyian Yari X24</Text>
          <Image
            source={require('../assets/yeyian.jpg')}
