...
const SkytechScreen = () => {
  ...
  const [totalPrice, setTotalPrice] = useState(1012.99);
  const { addToCart } = useCart(); // Get the addToCart function from CartContext

  const decreaseQuantity = () => {
    if (quantity > 1) {
      setQuantity(quantity - 1);
      setTotalPrice(totalPrice - 1012.99);
    }
  };
    ...
    >
      <View style={styles.container}>
        <View style={styles.productDetails}>
          {/* Product details */}
          <Text style={styles.productName}>Skytech Gaming Nebula</Text>
          <Image
            source={require('../assets/skytech.jpg')}
